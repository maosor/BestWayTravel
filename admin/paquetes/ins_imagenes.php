...
foreach ($_FILES['ruta']['tmp_name'] as $key => $value) {
  ...
  if ($info['extension']== "JPG" || $info['extension']== "jpg") {
    ...
    $copia = "fotos_paquete/".$renombrar.".jpg";
    ...
    $rand = rand(000,999);
    $renombrar =  $id.$rand.$cont;
    $copia = "fotos_paquete/".$renombrar.".png";
...
if ($ins) {
  header('location:../extend/alerta.php?msj=Imagenes guardadas&c=paq&p=img&t=success&id='.$id.'');
}else {
  header('location:../extend/alerta.php?msj=Imagenes no guardadas&c=paq&p=img&t=error&id='.$id.'');
}
$ins ->close();
$con->close();
}else {
